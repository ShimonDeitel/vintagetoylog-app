...
struct ContentView: View {
    ...
    @State private var editingItem: ToyItem?
    ...
    var body: some View {
        NavigationStack {
            ...
            .navigationTitle("Vintage Toy Log")
            .toolbar {
    ...
    @ViewBuilder
    private func row(for entry: ToyItem) -> some View {
        VStack(alignment: .leading, spacing: 4) {
            Text(entry.name)
                .font(Theme.headlineFont)
                .foregroundColor(Theme.textPrimary)
            Text("Condition: \(entry.condition)  \u{2022}  Box Included: \(entry.boxed)")
                .font(Theme.captionFont)
                .foregroundColor(Theme.textMuted)
        }
        .padding(.vertical, 4)
...
struct ItemEditView: View {
        ...
        NavigationStack {
            Form {
                Section {
                    ...
                    TextField("Condition (Near Mint)", text: $field1)
                        .focused($focusedField, equals: .field1)
                        .accessibilityIdentifier("field1Field")
                    TextField("Box Included (Yes)", text: $field2)
            ...
            .navigationTitle(mode == .add ? "Add Toy" : "Edit Toy")
            .toolbar {
    ...
    private func save() {
        switch mode {
        case .add:
            store.add(name: name, condition: field1, boxed: field2, notes: notes)
        case .edit(var entry):
            entry.name = name
            entry.condition = field1
            entry.boxed = field2
            entry.notes = notes
            store.update(entry)
        }
